...
$(document).ready(function() {
    ...
    $('.oneway_daterange').daterangepicker({
		...
		var startday = startDate.toLocaleString(locale,{weekday:'long'});
		
		var endDate = new Date(end);
		var endmonth = endDate.toLocaleString(locale,{month:'long'});
		var endday = endDate.toLocaleString(locale,{weekday:'long'});
	...
	var startdate = start_Date.toLocaleString(locale,{day:'2-digit'});
	var startmonth = start_Date.toLocaleString(locale,{month:'long'});
	var startday = start_Date.toLocaleString(locale,{weekday:'long'});
	...
	var enddate = end_Date.toLocaleString(locale,{day:'2-digit'});
	var endmonth = end_Date.toLocaleString(locale,{month:'long'});
	var endday = end_Date.toLocaleString(locale,{weekday:'long'});
	...
	$('#multi-city-flight').on('click' , '.add-multi-city-flight' , function() {
		...
		if(length == 1)
		{
			$(this).closest('#multi-city-flight').find('.flights_multi:nth-last-child(1)').after('<div class="col-md-8 flights_multi">'+
                '<h3 class="www-srchf__multi__ttl"><i class="fa fa-plane"></i> Flight '+ length+1 +' </h3>'+
                '<div class="row">'+
                   '<div class="col-md-9">'+
                      '<div class="row">'+
                         '<div class="col-md-12">'+
                            '<div class="custom-input-text top-radius">'+
                               ...
                               '<label class="custom_container">Nearby Airports<input type="checkbox" checked="checked">'+
                   ...
                   '<div class="col-md-3">'+
                      '<div class="row">'+
                         '<div class="col-md-12">'+
                            '<input type="text" class="daterange multicity_daterange_'+length+1+' " name="daterange" />'+
			...
			$(this).closest('#multi-city-flight').find('.flights_multi:nth-last-child(2)').after('<div class="col-md-8 flights_multi">'+
	                '<h3 class="www-srchf__multi__ttl"><i class="fa fa-plane"></i> Flight '+ ++length +' </h3>'+
	                '<div class="row">'+
	                   '<div class="col-md-9">'+
	                      '<div class="row">'+
	                         '<div class="col-md-12">'+
	                   ...
	                   '<div class="col-md-3">'+
	                      '<div class="row">'+
	                         '<div class="col-md-12">'+
	                            '<input type="text" class="daterange multicity_daterange_'+length+' " name="daterange" />'+
	                            '<div class="custom-input-text s_calendar square-radius square-radius-m start_date">'+
	                               '<i class="fa fa-calendar-o"></i>'+
	                               '<span class="month">'+startmonth+'</span>'+
	                               '<span class="date">'+startdate+'</span>'+
